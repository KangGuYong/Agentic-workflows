...
`allowPrivate`는 전역 스위치가 아니다. 그 항목이 매칭됐을 때만 면제가 적용되며, 다른 항목으로 매칭된 요청에는 영향이 없다. 동일한 host·port 항목이 중복으로 들어와 우선순위가 같으면 **권한이 낮은 쪽**(`allowPrivate` 없는 쪽)이 이긴다 — allowlist 두 개를 합칠 때 넓은 항목이 좁은 항목을 조용히 승격시키는 것을 막는다.

면제되는 범주는 **`private`, `loopback`, `cgnat` 세 개뿐이다**. 셋 다 "운영자 자신의 네트워크에 있는 기계"를 뜻한다. `link-local`은 **면제되지 않는다** — `169.254.169.254`는 클라우드 메타데이터 엔드포인트이고, 내부 API 하나를 열려고 `;allowPrivate`를 쓴 운영자가 메타데이터까지 열어달라고 한 것은 아니다. `multicast`, `unspecified`, `reserved`, `tunneled`, `non-global`, `invalid`도 면제 대상이 아니다.

(최초 설계는 `private` 하나만 면제했는데, 그러면 `127.0.0.1`이 `loopback`으로 분류되어 로컬 개발·테스트 서버를 가리키는 항목이 전부 막힌다.)
...
| reserved | `240.0.0.0/4`, `192.0.0.0/24`, `198.18.0.0/15` | — |
| tunneled | `192.88.99.0/24` (6to4 relay) | `2002::/16`, `2001::/32`, `64:ff9b::/96`, `64:ff9b:1::/48` |
| non-global | 표에 없지만 전역 라우팅되지 않는 모든 주소 | 〃 |
| invalid | 파싱되지 않는 문자열 | 〃 |

**IPv4-mapped/embedded IPv6는 풀어서 내부 주소를 다시 분류한다** — `::ffff:127.0.0.1`은 고전적인 우회 경로다. 풀어야 하는 형식은 하나가 아니다: `::ffff:0:0/96`(mapped) 외에 `::/96`(IPv4-compatible), `::ffff:0:0:0/96`(SIIT translated), NAT64 접두사가 모두 IPv4 주소를 품는다.

**분류는 default-deny다** (Task 1 리뷰 결과 반영). 표는 차단된 주소에 **이름을 주기 위한 것**이고, 표에 없는 주소는 전역 라우팅될 때만 통과한다. 표만으로 차단 목록을 만들면 IANA가 범위를 추가할 때마다 구멍이 하나씩 생긴다 — 실제로 최초 구현은 `::169.254.169.254`, `::ffff:0:169.254.169.254`, `64:ff9b:1::a9fe:a9fe`를 전부 "공인 주소"로 판정했다. 표를 유지하는 이유는 두 가지다: 범주 이름이 진단에 필요하고, 파이썬 패치 버전에 따라 `ipaddress`의 판정이 바뀐 전례가 있어 알려진 위험 범위는 인터프리터와 무관하게 고정해야 한다.

`non-global`은 표에 이름이 없는 차단을 가리킨다. 이름이 겹치면(예: 표의 `reserved`를 fallback 이름으로도 쓰면) 표 항목의 접두사 길이를 검증할 수 없게 되므로 **의도적으로 다른 이름**이다.
