...
- ❌ = 검증 **오류**(실행 불가), ⚠️ = **경고**(실행 가능).
- `| default(x)`는 **값이 없을 때만** `x`로 바꾸고 `null`은 그대로 둔다(Jinja 동작). `null`도 바꾸려면 `| default(x, true)`를 쓴다. 검증은 참조한 값의 형식에 `x`의 형식을 더해 판정한다(예: 숫자 칸에 `default('')` → 오류).
