...
편집기에는 아직 가져오기 화면이 없어서 API로 넣는다. 엔진이 떠 있어야 한다
...
그다음 편집기에서 `http://localhost:3000/workflows/$ID`를 열면 캔버스에 그려져 있다. 노드 위치까지
파일에 들어 있어서 따로 정렬하지 않아도 된다. 실행은 편집기의 `실행` 버튼이 가장 편하다.
