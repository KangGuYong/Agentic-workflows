...
워크플로 목록(`http://localhost:3000`)의 **`가져오기`** 버튼에서 이 폴더의 `.json` 파일을 고른다.
새 워크플로가 하나 만들어지고 편집기가 바로 열린다. 파일 이름이 워크플로 이름이 되고, 노드 위치까지
파일에 들어 있어서 따로 정렬하지 않아도 된다.

가져오기는 **항상 새로 만든다** — 열려 있는 워크플로를 덮어쓰지 않는다. 파일을 잘못 골라도 최악의
경우가 워크플로 하나 지우는 일이다.
