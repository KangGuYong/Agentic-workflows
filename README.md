...
2. **워크플로 만들기** — 목록에서 `새 워크플로`. 시작과 끝 노드만 있는 캔버스가 열린다.
   이미 있는 워크플로 파일이 있다면 목록의 `가져오기`로 `.json`을 고르면 새 워크플로로 열린다 —
   `examples/`에 바로 써 볼 수 있는 여섯 개가 있다. 편집기 툴바의 `가져오기`는 다르게 동작한다:
   **보고 있는 워크플로를 그 파일 내용으로 채운다** — 노드도 연결도 그대로 들어오고, 기존 내용은
   `되돌리기` 한 번으로 돌아온다.
3. **노드 놓고 잇기** — 왼쪽 팔레트에서 `템플릿`을 캔버스로 끌어다 놓고, 노드 오른쪽 손잡이에서
   다음 노드 왼쪽 손잡이로 끈다. 편집은 **1분에 한 번** 자동 저장되고, `Ctrl+C`(Mac은 `⌘C`)를
   누르면 바로 저장된다(오른쪽 위에 `저장됨`). 실행 버튼도 실행 전에 먼저 저장한다.
