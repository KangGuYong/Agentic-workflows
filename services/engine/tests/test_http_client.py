...
def _capturing(body: bytes = b"ok", headers: str = "") -> tuple[object, list[str]]:
    """Like `_responder`, but records the decoded request head of every connection into the returned
    list instead of discarding it -- the one shape behind every test below that needs to assert on what
    was actually sent on the wire. Always answers 200 text/plain; a test that cares about the response
    itself, not the request, should use `_responder` instead."""
    captured: list[str] = []
...
async def _redirect_pair(*, extra_location_headers: str = "") -> tuple[int, object, int, object, list[str]]:
    """Two servers wired for a cross-origin redirect: A always answers 302 with a Location pointing at
    B; B is a `_capturing` responder. Returns (port_a, server_a, port_b, server_b, captured) -- a test
    needs only build its own allowlist/resolver naming both ports and make the one request."""
    handle_b, captured = _capturing()
    port_b, server_b = await _serve(handle_b)

    async def handle_a(reader, writer):
        try:
            await reader.readuntil(b"\r\n\r\n")
            location = f"http://other.internal.test:{port_b}/landed"
            resp = (f"HTTP/1.1 302 Found\r\nLocation: {location}\r\n{extra_location_headers}"
                    f"Content-Length: 0\r\nConnection: close\r\n\r\n")
    ...
    return port_a, server_a, port_b, server_b, captured
...
# ---------------------------------------------------------------------------------------------------
# Allowlist and DNS ordering: the allowlist runs before DNS, on every hop, for every kind of host.
# ---------------------------------------------------------------------------------------------------
...
async def test_an_allow_entry_with_an_empty_host_is_still_rejected_before_dns():
    """AllowEntry is a public export, and nothing stops some future entry source from building one
    directly with host="" (parse_allowlist itself never does). White-box construct one and confirm an
    empty request-side host is refused before DNS, whichever of _resolve_target's two guards actually
    fires.

    Both the dedicated "if not host" guard and the has_hostname_syntax("") check further down reject
    the empty string, so this test cannot tell them apart -- and, having checked, removing either guard
    alone still leaves the other one catching this exact case, making each individually redundant here.
    Both stay anyway: see the comment on the "if not host" guard in _resolve_target for why."""
    resolver = FakeResolver({"": ["10.0.0.5"]})
    client = GuardedClient((AllowEntry(scheme="https", host="", port=443, allow_private=False),),
                           resolver=resolver)

    with pytest.raises(EgressBlocked) as exc:
        await client._resolve_target("https:///x")
...
async def test_a_syntactically_invalid_host_does_not_pass_a_wildcard_suffix_match(host):
    ...
    request-side syntax check in _resolve_target exists so that only a well-formed name reaches that
    match."""
    resolver = FakeResolver({})
...
async def test_a_syntactically_valid_wildcard_match_still_reaches_dns():
    """The companion to the test above: has_hostname_syntax must not be so strict that it blocks a
...
# ---------------------------------------------------------------------------------------------------
# Address classification: every resolved address is checked, an IP literal is checked too, and
# allowPrivate opens only the entry it is written on.
# ---------------------------------------------------------------------------------------------------
...
], ids=["forbidden-last", "forbidden-first"])
async def test_every_resolved_address_is_validated_regardless_of_order(addresses):
    """Two mutants earn their place in this one parametrized test: checking only the last resolved
    address would pass the forbidden-first case (the last address is fine) while missing forbidden-last
    entirely; checking only the first address is the mirror image. Only validating every address, in
    either order, catches both."""
    resolver = FakeResolver({"api.example.com": addresses})
    client = _client("https://api.example.com", resolver)
...
# ---------------------------------------------------------------------------------------------------
# TLS and connection setup: the pinned address is what's connected to, but the original host name is
# what TLS and the wire still see -- and one hostname's connection is never reused for another's.
# ---------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------
# Redirects: every hop is re-checked from scratch, a downgrade is refused, and a bodyless conversion
# drops the headers that no longer describe anything.
# ---------------------------------------------------------------------------------------------------
...
async def test_authorization_does_not_reach_a_cross_origin_redirect_target():
    """Pins that _follow_redirects actually calls headers_for_redirect -- not merely that the function
    itself is correct in isolation (see test_http_headers.py). Replacing that call with a no-op
    dict(headers) still passes every other test in this file; only checking the SECOND server's own
    captured wire catches it, since the cookie tests below cover the jar mechanism, not the caller's own
    Authorization header."""
    port_a, server_a, port_b, server_b, captured = await _redirect_pair()
...
async def test_a_redirect_to_a_malformed_port_is_blocked():
    ...
    around the Location-URL handling in _follow_redirects(), not the generic safety net."""
    port, server = await _serve(_responder(b"", status="302 Found",
...
async def test_a_302_redirect_converts_post_to_a_bodyless_get():
    ...
    # Not just that the method changed -- a conversion that changed the method but left a stale
    # Content-Length sitting on the now-bodyless GET would still pass the assertion above.
    assert "content-length" not in captured[1].lower()
    await client.aclose()


# ---------------------------------------------------------------------------------------------------
# Headers on the wire: what a tenant sets is sanitized, what this client pins cannot be overridden,
# and the URL's own query string reaches the server untouched.
# ---------------------------------------------------------------------------------------------------


async def test_tenant_supplied_framing_headers_never_reach_the_wire():
    handle, captured = _capturing(body=b"")
...
@pytest.mark.parametrize("headers", [
    {"X-Foo": "bar\r\nX-Injected: evil"},  # exhaustively covered in test_http_headers.py -- this file's
    {"X-Foo": 123},                        # job is only to pin the EgressBlocked("header") mapping and
])                                         # that DNS is never reached, not re-prove the header grammar
...
async def test_a_caller_supplied_host_header_cannot_override_the_pinned_host():
    ...
    host_lines = [line for line in captured[0].split("\r\n") if line.lower().startswith("host:")]
...
async def test_the_query_string_reaches_the_server():
    ...
    request_line = captured[0].splitlines()[0]
    assert request_line == "GET /x?a=1&b=2 HTTP/1.1"
    await client.aclose()


# ---------------------------------------------------------------------------------------------------
# Cookies: the jar is real (httpx's own doing), but nothing it ever holds reaches the wire, and it is
# cleared on every hop -- success, redirect, or refusal alike.
# ---------------------------------------------------------------------------------------------------


async def test_a_set_cookie_does_not_survive_a_cross_origin_redirect():
    port_a, server_a, port_b, server_b, captured = await _redirect_pair(
        extra_location_headers="Set-Cookie: sess=LEAKED\r\n")
    resolver = FakeResolver({"origin.internal.test": ["127.0.0.1"], "other.internal.test": ["127.0.0.1"]})
    client = _client(
        f"http://origin.internal.test:{port_a};allowPrivate, "
        f"http://other.internal.test:{port_b};allowPrivate",
...
async def test_a_set_cookie_does_not_survive_to_a_later_separate_request():
    handle, captured = _capturing(headers="Set-Cookie: sess=FIRST\r\n")
...
async def test_the_cookie_jar_itself_is_empty_after_a_response_with_set_cookie():
    """Isolates the jar clear in _follow_redirects from the Cookie-header override in _send: even
    though _send never trusts the jar for what goes out on the wire, the jar object itself must not
    keep growing -- removing self._client.cookies.clear() would leave this assertion failing even
    though no cookie would (yet) have reached any wire, because nothing has asked the jar to attach
    itself again."""
    port, server = await _serve(_responder(b"ok", headers="Content-Type: text/plain\r\n"
...
async def test_a_preexisting_jar_cookie_never_reaches_the_wire():
    """Isolates the Cookie-header pop in _send from the jar clear in _follow_redirects: seed the jar
    directly (as if a previous response's Set-Cookie had not been cleared in time, or anything else put
...
# ---------------------------------------------------------------------------------------------------
# Response reading: the size cap is streamed against the wire bytes, unsupported content is reported
# clearly, and the body is decoded only for the content types the DSL type system can hold.
# ---------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------
# Limits: request and response byte caps, the redirect budget, and the connection pool's own caps.
# ---------------------------------------------------------------------------------------------------
...
async def test_the_connection_pool_still_caps_total_connections():
    """httpx.Limits() resets max_connections to unlimited if constructed to change anything else -- see
    the module docstring for why. This reads the pool's own configuration back because there is no
    public accessor for it: `_transport._pool._max_connections`/`_max_keepalive_connections` are
    private httpx attributes, not a stable contract, and may need updating if a future httpx upgrade
...
# ---------------------------------------------------------------------------------------------------
# Failure mapping: the client's contract is exactly four exception types, and a bug in this module's
# own logic fails closed and loud rather than reaching the socket or being mistaken for a flaky network.
# ---------------------------------------------------------------------------------------------------
